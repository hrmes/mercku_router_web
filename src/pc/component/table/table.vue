<template>
  <div class="m-table-block">
    <div class="m-table-title" v-if="$slots.title || title">
      <slot name="title">{{title}}</slot>
    </div>
    <div class="m-table-slot">
      <slot></slot>
    </div>
    <div class="m-table-header" ref="tableHeader">
      <m-table-header ref="head" :columns="doColumns" :width="doWidth" :border="border" :headHeight="toCssVal(headHeight)" :has-gutter="hasGutter" :gutter-width="scrollWidth" @col-resize="thColResize"></m-table-header>
    </div>
    <div class="m-table-body" :style="{
        height: toCssVal(height),
        maxHeight: toCssVal(maxHeight),
        width: toCssVal(width),
        maxWidth: toCssVal(maxWidth)
      }" @scroll="onscroll" ref="tableBody">
      <table class="m-table" ref="table" :class="{
...
<script>
import MTableHeader from './table-header.vue';
...
export default {
  ...
  methods: {
    toCssVal(val) {
      ...
      if (/(px|rem|rem)/.test(val.toString())) {
        return val;
      }
      return `${val}px`;
    },
    ...
    docMousemove(e) {
      if (this.isColResize) {
        ...
        // 默认index列最新40，其他80
        const minWidth =
          parseInt(this.currentColumn.minWidth, 10) ||
          (this.currentColumn.index ? 40 : 80);
