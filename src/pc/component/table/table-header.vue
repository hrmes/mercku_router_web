<template>
  <table class="m-table m-table-head" :class="{
    ...
    <thead ref="thead">
      <th class="m-table-column" colspan="1" rowspan="1" v-for="(column, index) in columns" :key="column.label + index" :style="{
          width: column.width,
          height: headHeight
        }" :class="cellClass(column, index)" @mousemove="thMouseMove" @mousedown="thMouseDown($event, index)">{{column.label}}</th>
